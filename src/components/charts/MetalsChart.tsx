// src/components/charts/MetalsChart.tsx
...
> = {
  gold: { name: "Gold", color: "#FFD700", symbol: "XAU" },
  silver: { name: "Silver", color: "#C0C0C0", symbol: "XAG" },
  platinum: { name: "Platinum", color: "#E5E4E2", symbol: "XPT" },
  palladium: { name: "Palladium", color: "#B0C4DE", symbol: "XPD" },
};
...
function toChartData(payload: any): ChartPoint[] {
  ...
  const out = source
    .map((row: any): ChartPoint | null => {
      if (Array.isArray(row)) {
        const t = parseChartTimestamp(row[0]);
      ...
      if (row && typeof row === "object") {
        const t = parseChartTimestamp(
          row.t ?? row.time ?? row.timestamp ?? row.datetime ?? row[0]
        );

        const price = Number(
          row.p ?? row.price ?? row.value ?? row.c ?? row.close ?? row[1]
        );
...
function getChange(data: ChartPoint[]) {
  ...
  return { change, percent: (change / first) * 100 };
}
...
function generateLocalTimeTicks(min: number, max: number, hours: number): number[] {
  const ticks: number[] = [];
  ...
  if (remainder !== 0) {
    start.setHours(start.getHours() + (hours - remainder));
  }

  let cursor = start.getTime();

  while (cursor <= max) {
    if (cursor >= min) ticks.push(cursor);
...
function generateLocalDayTicks(
  ...
  while (start.getTime() < min) {
    start.setDate(start.getDate() + everyDays);
  }
  ...
  while (cursor <= max) {
    ...
    const nextDate = new Date(cursor);
    nextDate.setDate(nextDate.getDate() + everyDays);
...
function generateLocalMonthTicks(min: number, max: number): number[] {
  const ticks: number[] = [];
  const start = new Date(min);
...
function getXAxisTicks(points: ChartPoint[], totalDays: number): number[] {
  ...
  if (totalDays <= 1) return generateLocalTimeTicks(min, max, 4);
  if (totalDays <= 3) return generateLocalTimeTicks(min, max, 6);
  if (totalDays <= 14) return generateLocalDayTicks(min, max, 2);
  if (totalDays <= 90) return generateLocalDayTicks(min, max, 7);
  if (totalDays <= 180) return generateLocalDayTicks(min, max, 14);

  return generateLocalMonthTicks(min, max);
}
...
function ChartTooltip({
  ...
  return (
    <div className="min-w-[165px] rounded-xl border border-border/70 bg-popover/95 px-3 py-2.5 shadow-xl backdrop-blur-md">
      <div className="mb-1 text-[11px] font-medium text-muted-foreground">
        {date.toLocaleString(undefined, {
          month: "short",
          day: "numeric",
          year: "numeric",
          hour: totalDays <= 3 ? "numeric" : undefined,
          minute: totalDays <= 3 ? "2-digit" : undefined,
        })}
      </div>
...
export default function MetalsChart({
  ...
  const normalizedCurrency = ["USD", "EUR", "GBP"].includes(
  ...
  const url =
    ...
    `&symbol=${encodeURIComponent(`${meta.symbol}/${normalizedCurrency}`)}` +
  ...
  const rawChartData = React.useMemo(() => toChartData(data), [data]);

  const chartData = React.useMemo(
    () => filterChartPointsByWindow(rawChartData, range),
    [rawChartData, range]
  );
  ...
  const gradientId = `metal-gradient-${metal}`;

  const xAxisTicks = React.useMemo(
    () => getXAxisTicks(chartData, selected.days),
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: `linear-gradient(90deg, transparent, ${chartColor}, transparent)`,
        }}
      />
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-2.5">
              <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-muted/60 ring-1 ring-border/60">
                <span
            ...
            <div className="mt-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
              <span className="text-2xl font-bold tracking-tight sm:text-3xl">
                {latestPrice !== null
                  ? formatFullMoney(latestPrice, normalizedCurrency)
                  : "—"}
              </span>
            ...
            {chartData.length > 1 && (
              <div className="mt-1 text-xs text-muted-foreground">
                {isPositive ? "+" : ""}
                {formatFullMoney(change, normalizedCurrency)} over {selected.label}
              </div>
            )}
          </div>

          {/* Live indicator */}
          <div className="flex shrink-0 items-center gap-1.5 rounded-full border border-border/60 bg-muted/40 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <div
            ...
            className="flex max-w-full overflow-x-auto rounded-lg bg-muted/50 p-1 scrollbar-none"
          >
            {RANGES.map((item) => {
        ...
        <div
          className="
            relative -mx-5 mt-3 h-[290px] w-[calc(100%+2.5rem)]
            sm:mx-0 sm:h-[300px] sm:w-full
          "
        >
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis
                  ...
                  minTickGap={selected.days <= 3 ? 36 : 28}
                ...
                <YAxis
                  domain={["auto", "auto"]}
                  tickFormatter={(value) =>
                    formatMoney(Number(value), normalizedCurrency)
                  }
                  orientation="right"
                  width={58}
                  tick={{ fill: "var(--muted-foreground)", fontSize: 11 }}
        ...
        <div className="mt-1 flex items-center justify-between border-t border-border/50 pt-3">
          ...
          <div
            className={`
              text-[11px] font-medium transition-all duration-700
              ${
                justUpdated
