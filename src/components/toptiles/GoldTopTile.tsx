...
// fetcher in GoldTopTile.tsx and MetalsChart.tsx
...
export default function GoldTopTile() {
  ...
  const item =
    data && Array.isArray((data as any).items)
      ? (data as any).items.find(
          (it: any) =>
            String(it?.symbol || "").startsWith("XAU/") ||
            String(it?.name || "").toLowerCase() === "gold"
        )
      : null;
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="rounded-2xl bg-card text-card-foreground p-4 shadow-sm ring-1 ring-border">
        <div className="text-xs font-medium text-muted-foreground">Gold (XAU) Price</div>
