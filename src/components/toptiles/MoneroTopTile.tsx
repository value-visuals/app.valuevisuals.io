...
export default function MoneroTopTile({ stats, fetcher, className = "" }: TileProps) {
  ...
  useEffect(() => {
    ...
    effectiveFetcher()
      ...
      .catch((e) => {
        console.error(e);
        if (mounted) setError("Failed to load XMR data");
      });
  ...
  return (
    <div className={`grid gap-4 sm:grid-cols-2 lg:grid-cols-4 ${className}`}>
      <div className={tile}>
        <div className="text-sm text-muted-foreground">XMR Price</div>
      ...
      <div className={tile}>
        <div className="text-sm text-muted-foreground">XMR Market Cap</div>
      ...
      <div className={tile}>
        <div className="text-sm text-muted-foreground">XMR 24h Volume</div>
        <div className="mt-1 text-2xl font-semibold text-[var(--foreground)] truncate">
      ...
      <div className={tile}>
        <div className="text-sm text-muted-foreground">XMR Dominance</div>
        <div className="mt-1 text-2xl font-semibold text-[var(--foreground)]">
...
export async function fetchBitcoinStats() {
  const res = await fetch("/api/crypto/monero", { cache: "no-store" });
