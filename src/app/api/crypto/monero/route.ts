// src/app/api/crypto/monero/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    const [sumRes, globalRes] = await Promise.all([
      fetch(`${API_BASE}/api/crypto/summary?symbols=XMR`, {
    ...
    if (sumRes.ok) {
      ...
      const row = Array.isArray(summary?.data)
        ? summary.data.find((x: any) => x.symbol === "XMR")
        : null;
      if (row) {
    ...
    if (globalRes.ok) {
      ...
      } else if (global?.xmrDominancePct != null) {
        dominancePct = Number(global.xmrDominancePct) / 100; // convert percent -> fraction
      }
    }
    ...
    console.error("[/api/crypto/monero] proxy failed:", err?.message || err);
